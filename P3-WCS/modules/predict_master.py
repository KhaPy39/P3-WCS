...
# ==========================================
# ✅ Prédiction simple (live)
# ==========================================
def predict_live(table, pred_table, delta_time):
    df = get_last_data_block(table)
    df = add_primary_kpis(df)
    X = df.drop(columns=["date"])
    X_last = X.iloc[-1:].replace([np.inf, -np.inf], np.nan).ffill().fillna(0)  # ✅ Correction FutureWarning
...
# ==========================================
def predict_all():
    for table, (pred_table, delta_time, _) in interval_to_table.items():
        try:
            predict_live(table, pred_table, delta_time)
        except Exception as e:
            print(f"⚠️ Erreur prédiction pour {table} : {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--interval", choices=["btc_t15", "btc_h", "btc_d", "all"], required=True)
    args = parser.parse_args()
    if args.mode == "batch":
        table = args.interval
        if table == "all":
            print("⚠️ Mode batch ne supporte pas 'all'.")
            sys.exit(1)
        pred_table, delta_time, steps = interval_to_table[table]
        print(f"⚡ Mode batch pour {table} → {steps} prédictions")
        ...
        if args.interval == "all":
            ...
            table = args.interval
            pred_table, delta_time, _ = interval_to_table[table]
            predict_live(table, pred_table, delta_time)
